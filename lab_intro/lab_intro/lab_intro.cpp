...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      int signed_x = (int) x;
      int signed_y = (int) y;
      int distance = (int) sqrt((signed_x - centerX)*(signed_x - centerX) + (signed_y - centerY)*(signed_y - centerY));
      double luminance = pixel->l * (1 - 0.005*distance);
      pixel->l = (luminance >= 0.0)? luminance: 0.0;
    }
  }
